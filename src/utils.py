...
def create_response(model: str, **kwargs):
    if model.startswith("lumiteh"):
        return create_lumiteh_response(model=model, **kwargs)
...
def create_lumiteh_response(**kwargs):
    lumiteh_url = os.getenv("LUMITEH_API_URL", "https://api.lumiteh.cc")
    url = os.path.join(lumiteh_url, "lumiteh", "completions")

    headers = {
        "Authorization": f"Bearer {os.getenv('LUMITEH_API_KEY')}",
        "Content-Type": "application/json",
    }
